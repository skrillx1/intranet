...
class AllLeavesController extends ControllerBase {
    ...
    public function displayLeaves() {
        ...
        foreach ($results as $result) {
            ...
            $approveLink = ['#markup' => $approve_disabled ? $this->t('<span class="disabled"><i class="bi bi-check2 approve-link"></i></span>') : Link::fromTextAndUrl($this->t('<i class="bi bi-check2"></i>'), $approveUrl)->toString()];
            $denyLink = ['#markup' => $deny_disabled ? $this->t('<span class="disabled"><i class="bi bi-x-lg deny-link"></i></span>') : Link::fromTextAndUrl($this->t('<i class="bi bi-x-lg"></i>'), $denyUrl)->toString()];
