...
class PendingLeavesController extends ControllerBase {
    ...
    public function displayLeaves() {
        ...
        foreach ($results as $result) {
            ...
            $acceptLink = ['#markup' => $accept_disabled ? $this->t('<span class="accept-link disabled">Accept</span>') : Link::fromTextAndUrl($this->t('Accept'), $acceptUrl)->toString()];
    ...
    public function viewLeave($id) {
        ...
        if (empty($result)) {
            ...
            return $this->redirect('pending_leaves.display_leaves');
        }

        $leaveType = !empty($result->other) ? $result->leave_type . ' - ' . $result->other : $result->leave_type;

        $backUrl = Url::fromRoute('pending_leaves.display_leaves');
    ...
    public function updateLeaveStatus($id, $status) {
        ...
        return new RedirectResponse(Url::fromRoute('pending_leaves.display_leaves')->toString());
    }

    public function acceptLeave($id) {
        return $this->updateLeaveStatus($id, 'In Progress');
    }
